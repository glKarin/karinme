...
public class Main extends JFrame {
	...
	private static final int MAX_ME_WIDTH = 800;
	private static final int MAX_HEIGHT = 480;
	private static final int MENU_ITEM_HEIGHT = 48;
	private static final int MENU_BAR_WIDTH = MAX_WIDTH - MAX_ME_WIDTH;
	...
	private JButton resizeButton = new JButton("R-S");
	...
	private ActionListener menuFullscreenListener = new ActionListener() {
		public void actionPerformed(ActionEvent ev) {
			DeviceDisplayImpl deviceDisplay = (DeviceDisplayImpl) DeviceFactory.getDevice().getDeviceDisplay();
			setDeviceSize(deviceDisplay, Main.MAX_ME_WIDTH, Main.MAX_HEIGHT);
			pack();
	...
	private StatusBarListener statusBarListener = new StatusBarListener() {
		public void statusBarChanged(String text) {
			...
			statusBar.setPreferredSize(new Dimension(Main.MENU_BAR_WIDTH, Main.MENU_BAR_HEIGHT));
	...
	public Main(DeviceEntry defaultDevice) {
		...
		menuBar.setPreferredSize(new Dimension(MENU_BAR_WIDTH, 144));

		JMenu menuFile = new JMenu("File");

		menuOpenMIDletFile = new JMenuItem("Open MIDlet File...");
		SetMenuItemSize(menuOpenMIDletFile);
		menuOpenMIDletFile.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_O, ActionEvent.CTRL_MASK));
		...
		SetMenuItemSize(menuOpenMIDletURL);
		...
		SetMenuItemSize(menuSaveForWeb);
		...
		SetMenuItemSize(menuSelectDevice);
		menuSelectDevice.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_D, ActionEvent.CTRL_MASK));
		...
		SetMenuItemSize(menuStopCapture);
		menuStopCapture.setEnabled(false);
		...
		SetMenuItemSize(menuMIDletNetworkConnection);
		...
		SetMenuItemSize(menuRecordStoreManager);
		menuRecordStoreManager.setState(false);
		...
		SetMenuItemSize(menuLogConsole);
		menuLogConsole.setState(false);
		...
		SetMenuItemSize(menuShowMouseCoordinates);
		...
		SetMenuItemSize(menuAbout);
		menuAbout.addActionListener(menuAboutListener);
		menuHelp.add(menuAbout);

		/*
		JMenu menuOther = new JMenu("Others");
		JMenuItem menuFullscreen = new JMenuItem("Fullscreen");
		menuFullscreen.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_F, ActionEvent.CTRL_MASK));
		menuFullscreen.addActionListener(menuFullscreenListener);
		menuOther.add(menuFullscreen);
		*/
		...
		statusPanel.add(this.resizeButton, "North");
		statusPanel.add(statusBar, "South");
		JPanel menuPane = new JPanel();
		statusPanel.add(menuPane);
		menuPane.setLayout(new javax.swing.plaf.basic.DefaultMenuLayout(menuPane, BoxLayout.Y_AXIS));
		menuPane.add(menuBar);
		
		{
			MEButton btn;

			btn = new MEButton("Open");
			btn.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent ev) {
					menuOpenMIDletFile.doClick();
				}
			});
			menuPane.add(btn);

			btn = new MEButton("&Full\nscreen");
			btn.addActionListener(menuFullscreenListener);
			menuPane.add(btn);

			btn = new MEButton("Device");
			btn.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent ev) {
					menuSelectDevice.doClick();
				}
			});
			menuPane.add(btn);

			btn = new MEButton("Close");
			btn.addActionListener(menuCloseMidletListener);
			menuPane.add(btn);

			btn = new MEButton("Exit");
			btn.addActionListener(menuExitListener);
			menuPane.add(btn);
		}

		JScrollPane scrollPane = new JScrollPane(statusPanel, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
		scrollPane.setPreferredSize(new Dimension(MENU_BAR_WIDTH, MAX_HEIGHT));
	...
	public static void main(String args[]) {
        ...
        gd.setFullScreenWindow(app);	
		{
			...
			DeviceDisplayImpl deviceDisplay = (DeviceDisplayImpl) DeviceFactory.getDevice().getDeviceDisplay();
			app.setDeviceSize(deviceDisplay, MAX_ME_WIDTH, MAX_HEIGHT);
		}
		app.setVisible(true);
	...
	private class MEButton extends JButton
	{
		public MEButton(String text)
		{
			...
			d = new Dimension(Main.MENU_BAR_WIDTH, Main.MENU_ITEM_HEIGHT);
			margin = new Insets(0, 0, 0, 0);
			//f = getFont();
			if(text.startsWith("&"))
			{
				t = text.substring(1);
				t = "<html><body>" + t.replaceAll("\n", "<br>") + "</body></html>";
			}
			else
				t = text;
			...
			setText(t);
			setPreferredSize(d);
			//setFont(new Font(f.getFontName(), f.getStyle(), 12));
			setMargin(margin);
		}
	}

	private void SetMenuItemSize(JMenuItem item)
	{
		item.setPreferredSize(new Dimension((int)item.getPreferredSize().getWidth() * 2, MENU_ITEM_HEIGHT));
	}
}
